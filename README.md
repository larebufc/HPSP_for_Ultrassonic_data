...
#### Function for sample from a discrete distribution and simulated indicator variables.
...
#### Exmplaned variables for fixed-effect for each individual.
...
## Simulated non-observed random variable for each group, which represent the effect of the group.
...
Function for sample from the posterior of parameter $\sigma^2$.
...
#### Useful informations for MCMC interactions.
...
#### Initial valures for  the MCMC chain.
...
```{r}
burnin<-1000         ## burnig
amostrasfin<-5000    ## sample used for inference without jump
...
MCMC_run<-fuction(Y,X,Z,burning,amostrasfin,saltos){
...
est.param=MCMC_run(Y,X,Z,burning,amostrasfin,saltos)
...
#### Graphical representation of estimated classification individuals.
